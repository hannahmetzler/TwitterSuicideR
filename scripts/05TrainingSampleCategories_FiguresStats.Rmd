...
d <- d %>% 
  ...
  mutate(category = factor(category, levels = ord_category, labels = ord_category),
         category_details = factor(category, levels = ord_category_details, labels = ord_category_details),
...
prop.main <- as.data.frame(d %>%
                             ...
                             mutate(freq = n / sum(n))) %>% 
  ungroup()
write.table(prop.main,'../output/tweets_per_maincategory_randomsample_20211004.tsv', sep='\t', row.names=F)
# prop.main
```
...
prop.cat = as.data.frame(d %>% 
                           ...
                           mutate(freq = round(n / sum(n)*100,2))) %>%
  ungroup()
# write.table(prop.cat,'../output/tweets_per_subcategory_randomsample_20211004.tsv', sep='\t', row.names=F)
```
