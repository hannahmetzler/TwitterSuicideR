...
#BERT predictions
db4 <- read.csv('../reliability_datasets/round4_150x5_suicide_withpredictions_BERT_not_finetuned.csv', stringsAsFactors = F, sep = '\t',  colClasses = "character") %>% 
  mutate(predictionBERT1 = as.factor(predictionBERT1),
         id = as.character(id)) %>% 
  rename(main_category.BERT = predictionBERT1)
#Hannah's labels
...
#Thomas' labels
dt4 <- read.csv('../reliability_datasets/round4_150x5_suicide_Thomas_complete_IDsrepaired.csv', stringsAsFactors = F, colClasses = "character") %>%
  select(-X1.problem.1) %>% 
  #delete text columns, messed up by excel, use text from BERT predictions dataset. #delete text columns split into multiple columns in Hannah file
...
# # write.csv2(file="../output/empty_labels_reliability_round4_Thomas.csv",d, row.names=F)
...
d4 = d4[, c("id", "time", "text", 
            "main_category.BERT", "category.hannah", "category.thomas", 
            "main_category.hannah",  "main_category.thomas", 
            "category2", "focus.hannah", "focus.thomas",   "type.hannah",
...
We need 21 more awareness tweets to have 100 per category. 

```{r,data for labelling more tweets}
#join missing labels and duplicate labels with new awareness tweet candidates
missing_ids = read.csv("../output/empty_labels_reliability_round4_Thomas.csv", colClasses = "character")$id
...
write.table(file="../output/labels_thomas_missing_duplicates.tsv", d, sep="\t", row.names = F)
