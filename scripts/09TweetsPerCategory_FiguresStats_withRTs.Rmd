...
Sys.setlocale("LC_ALL", 'en_US.UTF-8')
ggplot(data=dpaper, aes(x=date, y=percent, colour = prediction))+
  geom_line()+
  theme_bw()+
...
ggsave('../figures/Daily_timeseries_per_category.pdf',  width=10, height=13, dpi=300)
```
...
```{r}
dprev9 = dp1 %>%
  filter(main_category=="prevention" & lubridate::month(date)=="9" )
```
...
```{r, coping suicidality daily, fig.width=10, fig.height=4}
ggplot(data=filter(dpaper, prediction=="Coping" | prediction=="Suicidal ideation & attempts"), 
       aes(x=date, y=percent, colour = prediction))+
  geom_line()+ facet_wrap(~prediction, ncol=1)+
  theme_bw()+theme(legend.position="none")+
  scale_colour_manual(values=mycols2)+
  ylab("Percent per predicted category")+
  theme(legend.position="none", text=element_text(size=s+5), axis.title.x=element_blank(), plot.margin = margin(rep(0.5,4), unit="cm"))
# ggsave('../figures/Daily_timeseries_Coping_Suicidal.pdf',  width=10, height=5, dpi=300)
```
...
<!-- * 2018-08-31				 -->
* 2017-11-24/25: 	
  - Possible events: [Sinai mosque attack](https://en.wikipedia.org/wiki/2017_Sinai_mosque_attack)
* 2018-08-25: 
     - suicide attack in jalaad?	 [Aljazeera](https://www.aljazeera.com/news/2018/8/25/suicide-attack-in-jalalabad-claims-at-least-seven-lives)			



## Suicidal ideation 

Peaks above 2000 daily tweets for suicidal stories: 
...
dcatperday %>%
  ...
  select(prediction, date, percent) %>% 
  group_by(date) %>% slice(1) %>% 
  arrange(percent, decreasing = T)
```

## Prevention

```{r}
ggplotly(ggplot(data=filter(dpaper, prediction=="Prevention"), 
...
* October 2018 suicide cluster: https://www.newyorker.com/magazine/2021/04/19/a-mysterious-suicide-cluster
...
* 5  large Werther peaks: 
 - 2016-02-05: Mirra David died by suicide on February 4, 2016. [Wikipedia](https://en.wikipedia.org/wiki/Dave_Mirra), 
 and suicide attack in [Kabul](https://www.mfa.gov.tr/no_-35_-2-february-2016_-press-release-regarding-the-suicide-attack-perpetrated-in-kabul_-the-capital-city-of-afghanistan.en.mfa)
 - 2017-09-08: nobody on wiki list
 - 2017-10-05
