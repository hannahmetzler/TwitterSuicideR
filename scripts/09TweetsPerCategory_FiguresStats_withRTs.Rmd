...
```{r}
prop.pred.grand = as.data.frame(dcatperday %>%
                group_by(prediction) %>%
...
# write.csv(file="../results/estimated_freq_per_category_retweet_dataset.csv", prop.pred.grand)
prop.pred.grand
```
...
prop.pred.daily = dcatperday %>%
  ...
  #daily percentage
  summarise(freq = ncat/ndaytotal*100) %>% 
