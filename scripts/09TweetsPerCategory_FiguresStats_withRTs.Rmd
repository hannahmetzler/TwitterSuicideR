---
title: "Tweets per Category in the ML predictions dataset (BERT) based on query with RTs"
...
prop.pred$recall_bert = c(0.45, 0.69, 0.70, 0.89, 0.77, NA)
prop.pred = prop.pred %>% 
  mutate(n_adjusted = n/recall_bert) %>% 
  mutate(freq_adjusted = round(n_adjusted/sum(prop.pred$n)*100,2))
  #correct n for category irrelevant (because dividing by recall does not work there)
prop.pred[prop.pred$prediction=="irrelevant", "freq_adjusted"] = 100-sum(prop.pred$freq_adjusted, na.rm = T)
# write.csv(file="../output/estimated_freq_per_category_retweet_dataset.csv", prop.pred)
prop.pred
