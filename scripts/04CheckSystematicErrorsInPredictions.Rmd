...
```{r}
load('../reliability_datasets/round4_150x5_suicide_human&model_BERT_finetuned.R')
...
plotpercent = ggplot(main_confusion_matrix, aes(Prediction, Reference, fill = Freq)) +
  geom_tile() +
  geom_text(aes(label = scales::percent(Freq))) + #include only for png figure , size=fontsize-3
  scale_fill_gradient(low = "white", high = "#3575b5") +
  labs(x = "Prediction", y = "True Label", fill = "Percent") + theme_bw()+
  theme(plot.title = element_text(size = fontsize+11, hjust = 0.5, 
                                  margin = margin(20, 0, 20, 0)),
        legend.title = element_text(size = fontsize, margin = margin(0, 20, 10, 0)),
        axis.title.x = element_text( size = fontsize+4), #margin = margin(20, 20, 20, 20),
        axis.title.y = element_text(size = fontsize+4), #margin = margin(0, 20, 0, 10), 
...
plot_count = ggplot(main_confusion_matrix_count, aes(Prediction, Reference, fill = Freq)) +
  geom_tile() +
  geom_text(aes(label = (Freq))) + #include only for png figure , size=fontsize-3
  scale_fill_gradient(low = "white", high = "#3575b5") +
  labs(x = "Prediction", y = "", fill = "Percent") + theme_bw()+
  theme(plot.title = element_text(size = fontsize+11, hjust = 0.5, #+11
                                  margin = margin(20, 0, 20, 0)),
        legend.title = element_text(size = fontsize, margin = margin(0, 20, 10, 0)),
        axis.title.x = element_text( size = fontsize+4), #margin = margin(20, 20, 20, 20),
        axis.title.y = element_text(size = fontsize+4), #margin = margin(0, 20, 0, 10), 
        ...
        axis.text.x = element_text(angle = 45,  hjust=1), 
        legend.position = "none")
ggarrange(plotpercent, plot_count,  common.legend = TRUE, legend = "none", labels = c("a", "b"), font.label = list(size = 16))
ggsave('../figures/Main_category_confusion_matrix.pdf', width=10, height=5, dpi=300)
# ggsave('../figures/Figure2.png', width=4, height=2)
```
...
How many predicted labels per category were labelled as not serious by Coder 1? They should all belong to the category irrelevant, but are quite often assigned to the suicidal category. 
